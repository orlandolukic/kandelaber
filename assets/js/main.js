(function ($) {
	...
	var qodefSwiper = {
		...
		getOptions: function ($holder, returnBreakpoints) {
			var sliderOptions = typeof $holder.data('options') !== 'undefined' ? $holder.data('options') : {},
				...
				nextNavigation = outsideNavigation ? '.swiper-button-next-' + sliderOptions.unique : $holder.find('.swiper-button-next')[0],
				prevNavigation = outsideNavigation ? '.swiper-button-prev-' + sliderOptions.unique : $holder.find('.swiper-button-prev')[0],
			...
			if ( $holder.hasClass('qodef-slides-variable-width') ) {
                slidesPerView = 'auto';
    ...
    var qodefAppear = {
        init: function () {
            this.holder = $('.qodef--has-appear, .qodef-custom-appear');
    ...
    var qodefFloat = {
		init: function () {
			var $floatElement = $('.qodef-custom-float');

			if ( $floatElement.length ) {
				$floatElement.attr('data-parallax', '{"y": -50, "smoothness": 300}');
			}
...
(function ($) {
    ...
    $(document).on('lucent_trigger_get_new_posts', function (e, holder) {
	...
	var qodefFilter = {
		...
		setVisibility: function (holder, item, triggerEvent) {
			...
			if (!isShowAllFilter && !listHasVisibleItems && qodefFilter.hasLoadMore(holder)) {
				qodef.body.trigger('lucent_trigger_load_more', [holder]);
			} else {
...
(function ($) {
	...
	$(document).on('lucent_trigger_load_more', function (e, $holder, nextPage) {
				...
				success: function (response) {
					...
					if (response.status === 'success') {
						...
						$itemsHolder.waitForImages(function () {
							...
							if ($holder.hasClass('qodef-pagination-type--standard')) {
								$('html, body').animate({
									scrollTop: $holder.offset().top - 120
								}, 500, 'swing');
							}
							
							setTimeout(function () {
								qodef.body.trigger('lucent_trigger_get_new_posts', [$holder, response.data, nextPage]);
...
(function ($) {
	...
	$(document).ready(function () {
        ...
        qodefWooWidgetsCountMarkup.init();
    ...
    var qodefWooWidgetsCountMarkup = {
        init: function() {
            var countHolder = $('.widget.woocommerce li .count');

            if (countHolder.length) {

                countHolder.each(function () {
                    var thisContentHolder = $(this);
                    var thisContentLink = thisContentHolder.prev('a');
                    var thisContent = '';
                    var thisNewContent = '';

                    thisContent = thisContentHolder.text(); // initial text

                    thisNewContent = '<span class="count qodef-count-num"><sup>' + thisContent.substring(1, thisContent.length - 1) + '</sup></span>'; // remove parentheses and add sup tag

                    thisContentHolder.empty(); // empty old content
                    thisContentLink.wrapInner("<span class='qodef-count-link'></span>"); // add span around category name text
                    thisContentLink.append(thisNewContent); // add modified content inside link
                });
            }
        }
    };
